        ...
        */
        ...
        void SetupNewAtlasTexture()
        {
            ...
            if (UnityEditor.EditorUtility.IsPersistent(this))
            {
                ...
                // Fix TMP 3.0.9 bug: m_AtlasTexture (singular) is not serialized and is always null at runtime,
                // which throws UnassignedReferenceException here and prevents the new atlas from being persisted
                // (AddObjectToAsset below is never reached), leaving {fileID: 0} null atlas slots.
                // Use this.name (the font asset's own name) as the prefix instead of the null m_AtlasTexture.
                tex.name = this.name + " " + m_AtlasTextureIndex;
